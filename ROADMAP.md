...
`v0.1.23` is the public baseline. Phase 29's source-neutral contracts, secure local-file platform, owner-private Library migration `0022`, and capability gates merged through PR #39; Google Drive PKCE, encrypted token storage, isolated Picker, and range streaming merged through PR #41. The owner reports the migration and RLS SQL passed. `frontend/phase-29-library-ui` now provides capability-gated local/Drive selection, fingerprint progress/cancellation, opaque lease playback, and a responsive native-video Library workspace, with 268 tests passing. Capabilities remain off by default, and room playback remains hidden until `media:v1:*` synchronization is implemented. NightWatch does not download YouTube, extract protected services, bypass DRM, relay participant media, or host a shared catalog. `STATUS.md`, `TASKS.md`, `PHASE_29_BACKEND_STATUS.md`, and `CODEX_HANDOFF.md` remain authoritative for active work.
