...
`v0.1.23` is the public baseline. Phase 28 control/search, official muted hover previews, the same-instance mini-player, caption preferences, expanded customization, and responsive polish merged through PR #38. Phase 29's source-neutral contracts, secure Electron local-file platform, owner-private Library migration `0022`, and capability gates merged through PR #39 after 197 tests and Windows packaging passed. The owner reports the migration and RLS acceptance SQL passed. Local/Drive capabilities remain disabled until packaged playback acceptance and the matching UI land. Google Drive work continues separately on `backend/phase-29-drive`; room synchronization follows after Drive security review. NightWatch does not download YouTube, extract protected services, bypass DRM, relay participant media, or host a shared catalog. `STATUS.md`, `TASKS.md`, `PHASE_29_BACKEND_STATUS.md`, and `CODEX_HANDOFF.md` remain authoritative for active work.
