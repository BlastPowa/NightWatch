    ...
    <div className="creator-page fade-up">
      ...
      <div className="creator-workspace">
        <aside className="creator-club-rail card">
          ...
          {!loading && clubs.length === 0 && <div className="creator-empty"><span>◎</span><strong>Create the first club</strong><small>Clubs you own or join will live here.</small></div>}
        </aside>

        <main className="creator-board card">
          {selectedClub === null ? <div className="creator-empty creator-empty-large"><span>✦</span><strong>Your creator board is ready</strong><small>Create a club to start collecting and judging community video ideas.</small></div> : <>
            ...
            {visibleBounties.length === 0 && <div className="creator-empty"><span>◇</span><strong>Nothing in this view</strong><small>New challenges and submissions will appear here.</small></div>}
          </>}
        </main>

        <aside className="creator-detail card">
          {selectedBounty === null ? <div className="creator-empty creator-empty-large"><span>↗</span><strong>Select a bounty</strong><small>Open a challenge to see its submissions and voting state.</small></div> : <BountyDetail bounty={selectedBounty} results={results} onRefresh={() => { void getBountyResults(selectedBounty.id).then((result) => { if (result.status === 'ok') setResults(result.data); }); if (selectedClubId !== null) void refreshBounties(selectedClubId); }} onError={setError} />}
        </aside>
      </div>
    </div>
  );
