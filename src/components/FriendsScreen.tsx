...
export function FriendsScreen({ onMessage }: { onMessage(userId: string): void }): JSX.Element {
  ...
  return <section className="social-page fade-up" aria-labelledby="friends-title">
    ...
    {loading ? <div className="social-loading"><div className="orbit-loader" aria-hidden="true"><span /><span /><span /></div><span>Loading your circle…</span></div> : <div className="social-sections">
      {graph.incoming.length > 0 && <RelationSection title="Friend requests" subtitle="People waiting for your response" items={graph.incoming} onOpen={setProfileId} renderActions={(person) => <><button className="button button-primary" disabled={busyId === person.userId} onClick={() => void act(person.userId, () => acceptFriendRequest(person.userId), `${person.displayName} is now your friend.`)}>Accept</button><button className="button" disabled={busyId === person.userId} onClick={() => void act(person.userId, () => declineFriendRequest(person.userId), 'Request declined.')}>Decline</button></>} />}
      <RelationSection title="Your friends" subtitle="Accepted NightWatch connections" items={graph.friends} onOpen={setProfileId} presence={presence} empty="Watch together in persistent rooms to discover people you know." renderActions={(person) => <><button className="button button-primary" onClick={() => onMessage(person.userId)}>Message</button><button className="button" disabled={busyId === person.userId} onClick={() => void act(person.userId, () => removeFriend(person.userId), `${person.displayName} was removed.`)}>Remove</button></>} />
      {graph.suggestions.length > 0 && <RelationSection title="People you watched with" subtitle="Suggestions are not friends until they accept" items={graph.suggestions} onOpen={setProfileId} renderActions={(person) => <button className="button button-primary" disabled={busyId === person.userId} onClick={() => void act(person.userId, () => sendFriendRequest(person.userId), `Request sent to ${person.displayName}.`)}>Add friend</button>} />}
      {graph.outgoing.length > 0 && <RelationSection title="Sent requests" subtitle="Waiting for a response" items={graph.outgoing} onOpen={setProfileId} renderActions={(person) => <button className="button" disabled={busyId === person.userId} onClick={() => void act(person.userId, () => cancelFriendRequest(person.userId), 'Request cancelled.')}>Cancel request</button>} />}
      <section className="social-section"><header><div><h2>Blocked</h2><p>Blocking is mutual and total: you disappear from each other everywhere.</p></div></header><BlockedUsersPanel /></section>
    </div>}
    {profileId !== null && <SocialProfileCard userId={profileId} onClose={() => setProfileId(null)} onMessage={(id) => { setProfileId(null); onMessage(id); }} />}
  </section>;
}

function RelationSection({ title, subtitle, items, presence, empty, onOpen, renderActions }: { title: string; subtitle: string; items: readonly Relation[]; presence?: ReadonlyMap<string, FriendPresence>; empty?: string; onOpen?(userId: string): void; renderActions(person: Relation): JSX.Element }): JSX.Element {
  return <section className="social-section"><header><div><h2>{title}</h2><p>{subtitle}</p></div><span>{items.length}</span></header>{items.length === 0 ? <p className="social-empty">{empty}</p> : <ul className="social-grid">{items.map((person) => { const activity = presence?.get(person.userId); return <li key={`${person.kind}-${person.userId}`} className="person-card"><button type="button" className="person-avatar-button" onClick={() => onOpen?.(person.userId)} aria-label={`View ${person.displayName}'s profile`}><ProfileAvatar name={person.displayName} src={person.avatarUrl} className={person.selectedBorderId !== null ? `person-avatar border-${person.selectedBorderId}` : 'person-avatar'} />{activity !== undefined && activity.status !== 'offline' && <i className={`presence-dot presence-${activity.status}`} />}</button><span className="person-copy"><strong>{person.displayName}</strong><small>{activity !== undefined && activity.status !== 'offline' ? activity.status === 'in_party' ? 'In a watch party' : activity.status === 'watching' ? activity.videoTitle ?? 'Watching now' : 'Online' : person.kind === 'suggestion' ? 'Co-watcher suggestion' : person.kind === 'friend' ? 'Friend · presence private' : 'NightWatch member'}</small></span><span className="person-actions">{renderActions(person)}</span></li>; })}</ul>}</section>;
}
